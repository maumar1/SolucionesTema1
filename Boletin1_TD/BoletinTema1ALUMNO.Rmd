...
```{r}
#Como están escritos de difetente forma no salen en los dos conjuntos.
```
...
#Ambos gráficos presentan un comportamiento muy parecido.
#Sí, podemos decir que siguen una distribución Gaussiana con un 95% de confianza
```
...
#4b
sum(is.na(cabbages)) # 0 valores perdidos en el conjunto cabbages
...
sum(is.na(Chile)) # 295 valores perdidos en el conjunto Chile
...
# La variable HeadWt tiene todos sus puntos entre las líneas discontinuas del qqPlot, por lo que podemos decir que todos los valores cumplen la distribución Gaussiana con una seguridad del 95%.

# La variable VitC tiene todos sus puntos en su totalidad dentro del qqPlot, por lo que podemos decir con una seguridad del 95% que cumplne la distribución Gaussiana.
```
...
# Es fácil ver que no hay una recta que se adapte al modelo
...
# En el gráfico, los rombos sombreados representan c39, y los puntos blancos a c52
```
